...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (realtimeManager && typeof realtimeManager.unsubscribe === 'function') {
    ...
    try {
      ...
      try {
        ...
        if (persistedState) {
          ...
          // Restore key parts of the state
  ...
  const updateStoreInfo = async (storeData: Partial<Store>) => {
    try {
      ...
      if (error) {
        console.error('Supabase updateStoreInfo error:', error);
  ...
  const loadProducts = async () => {
    ...
    const cacheKey = `products-${state.user.id}`;
    ...
    try {
      ...
      // Get products with user and store information
      ...
      dataConsistencyManager.updateCache(cacheKey, productsWithRelations);
      dispatch({ type: 'SET_PRODUCTS', payload: productsWithRelations });
      dispatch({ type: 'UPDATE_LAST_SYNC', payload: { key: 'products', value: Date.now() } });
      
      console.log('Products loaded successfully:', productsWithRelations.length);
    } catch (error) {
  ...
  const loadOrders = async () => {
    ...
    const cacheKey = `orders-${state.user.id}`;
    const cachedData = dataConsistencyManager.getCachedData(cacheKey);
    ...
    try {
      ...
      // Get orders with user and store information
      ...
      const ordersWithRelations = orders || [];
      
      dataConsistencyManager.updateCache(cacheKey, ordersWithRelations);
      dispatch({ type: 'SET_ORDERS', payload: ordersWithRelations });
      dispatch({ type: 'UPDATE_LAST_SYNC', payload: { key: 'orders', value: Date.now() } });
      
      console.log('Orders loaded successfully:', ordersWithRelations.length);
    } catch (error) {
  ...
  const loadPosts = async () => {
    ...
    const cacheKey = `posts-${state.user.id}`;
    const cachedData = dataConsistencyManager.getCachedData(cacheKey);
    ...
    try {
      ...
      // Get posts with user and store information
      ...
      const postsWithRelations = posts || [];
      
      dataConsistencyManager.updateCache(cacheKey, postsWithRelations);
      dispatch({ type: 'SET_POSTS', payload: postsWithRelations });
      dispatch({ type: 'UPDATE_LAST_SYNC', payload: { key: 'posts', value: Date.now() } });
      
      console.log('Posts loaded successfully:', postsWithRelations.length);
    } catch (error) {
  ...
  const createProduct = async (productData: Partial<Product>) => {
    // Development mode: Create mock product if no user
    if (!state.user?.id) {
      ...
      dispatch({ type: 'ADD_PRODUCT', payload: mockProduct });
      console.log('Mock product created successfully:', mockProduct);
      return mockProduct;
    }
    ...
    try {
      ...
      // Create the product with proper relationships
  ...
  const createPost = async (postData: Partial<Post>) => {
    try {
      // Development mode: Create mock post if no user
      if (!state.user?.id) {
        console.log('Development mode: Creating mock post');
        const mockPost: Post = {
          id: `mock-${Date.now()}`,
          user_id: 'mock-user-id',
          store_id: 'mock-store-id',
          ...
          featured_product_id: postData.featured_product_id || null,
          likes_count: 0,
          saves_count: 0,
          shares_count: 0,
        ...
        dispatch({ type: 'ADD_POST', payload: mockPost });
      ...
      // Create the post with proper relationships
      ...
      if (!post) {
        throw new Error('No post returned from database');
      }
  ...
  useEffect(() => {
    ...
    // Save state after a short delay to avoid excessive writes
    const timeoutId = setTimeout(saveState, 1000);
    return () => clearTimeout(timeoutId);
  }, [state.products, state.orders, state.posts, state.notifications]);
