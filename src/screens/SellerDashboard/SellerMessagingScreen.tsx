...
export default function SellerMessagingScreen() {
  ...
  const loadConversations = useCallback(async () => {
    try {
      ...
      // const response = await fetch('/api/seller/messaging/conversations');
  ...
  const handleConversationPress = (conversation: Conversation) => {
    console.log('Seller navigating to chat with conversation:', conversation.id);
    navigation.navigate('SellerChat', { conversationId: conversation.id });
  };
  ...
  const renderConversationItem = ({ item }: { item: Conversation }) => (
    ...
    >
      <View style={styles.avatarContainer}>
        <Image
          source={{ uri: item.users?.avatar_url || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=100&q=80' }}
          style={styles.avatar}
        />
        {item.unread_count > 0 && (
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtitle}>
        When customers message you about products or orders, they'll appear here
      </Text>
    </View>
  );
