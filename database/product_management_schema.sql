...
CREATE TABLE IF NOT EXISTS product_inventory_history (
    ...
    -- Notes
    notes TEXT,
    created_by UUID, -- Will reference user_profiles(id) when that table exists
...
CREATE POLICY "Store owners can manage their variants" ON product_variants
...
-- Brand Policies
CREATE POLICY "Public can view active brands" ON product_brands
    FOR SELECT USING (is_active = true);
