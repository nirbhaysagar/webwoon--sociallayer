-- Product Management Database Schema - FIXED VERSION
...
-- Product variants (enhanced) - Updated to use UUID for product_id to match existing schema
...
-- Product images (enhanced) - Updated to use UUID for product_id
...
-- Product attribute assignments - Updated to use UUID for product_id
...
-- Product tag assignments - Updated to use UUID for product_id
...
-- Product reviews - Updated to use UUID for product_id and order_id
...
-- Product favorites - Updated to use UUID for product_id
...
-- Product views tracking - Updated to use UUID for product_id
...
-- Product inventory history - Updated to use UUID for product_id
...
-- Product price history - Updated to use UUID for product_id
...
-- Simplified policy for store owners to manage their products
CREATE POLICY "Store owners can manage their products" ON public.products
    FOR ALL USING (auth.uid() = (SELECT owner_id FROM public.stores WHERE id = products.store_id));

-- Categories policies
CREATE POLICY "Users can view active categories" ON public.product_categories
    FOR SELECT USING (is_active = TRUE);

-- Simplified policy for store owners to manage categories
...
-- Simplified policy for store owners to manage variants
CREATE POLICY "Store owners can manage variants" ON public.product_variants
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.products p
            WHERE p.id = product_variants.product_id
            AND auth.uid() = (SELECT owner_id FROM public.stores WHERE id = p.store_id)
        )
    );
...
-- Simplified policy for store owners to manage images
...
-- Simplified policy for store owners to manage attributes
...
-- Simplified policy for store owners to manage attribute values
...
-- Simplified policy for store owners to manage attribute assignments
...
-- Simplified policy for store owners to manage tags
...
-- Simplified policy for store owners to manage tag assignments
...
-- Simplified policy for store owners to manage reviews
...
-- Simplified policy for store owners to view analytics
...
-- Simplified policy for store owners to view inventory history
...
-- Simplified policy for store owners to view price history
...
-- Function to get product with all related data - Updated to use UUID
CREATE OR REPLACE FUNCTION get_product_with_details(product_uuid UUID)
RETURNS TABLE (
    id UUID,
    name TEXT,
    description TEXT,
...
-- Function to search products - Updated to use UUID
...
-- Function to get product analytics - Updated to use UUID
...
GRANT EXECUTE ON FUNCTION get_product_with_details(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION search_products(TEXT, UUID, DECIMAL, DECIMAL, TEXT[], TEXT[], TEXT, TEXT, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_product_analytics(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION generate_product_slug(TEXT, UUID) TO authenticated;
